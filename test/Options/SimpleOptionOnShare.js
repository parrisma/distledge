...
/**
 * This suite tests the SimpleOptionOnShare
 */

describe("Simple Option On Share Test Suite", function () {
  // Define accounts
  let escrow_manager;
  ...
  let settlementToken; // ERC20StableShare
  // Price sources
  let equityPrice;
  let fxPrice;
  // Option factory
  let SimpleOptionOnShare;
  ...
  it("Valuate contract as positive return when spot is greater than strike", async function () {
    ...
    var optionOnShare = await getSimpleOptionOnShare();

    expect(await optionOnShare.connect(option_buyer).valuation()).to.equal(
      2000000
    );
  });
  ...
  it("Valuate contract as zero when spot is equal to strike", async function () {
    ...
    expect(await optionOnShare.connect(option_buyer).valuation()).to.equal(0);
  });
  ...
  it("Settlement amount changed when underlying price changed", async function () {
    ...
    expect(await optionOnShare.connect(option_seller).valuation()).to.equal(
      2000000 // Value before decimals handling
    );
    expect(
      await optionOnShare.connect(option_seller).settlementAmount()
  ...
  it("Balance of seller account reduced after exercise", async function () {
    ...
    var settlementAmount = await optionOnShare
      .connect(option_seller)
      .settlementAmount();
    await settlementToken
      .connect(option_seller)
      .approve(optionOnShare.contractAddress(), settlementAmount);
    console.log(
      "Settlement amount for SimpleOptionOnShare is " + settlementAmount
    );

    expect(await optionOnShare.connect(option_buyer).exercise()).to.emit(
      optionOnShare,
      "Exercised"
    );
  ...
  beforeEach(async function () {
    ...
    SimpleOptionOnShare = await ethers.getContractFactory(
      "SimpleOptionOnShare"
    );
  });
  ...
  async function getSimpleOptionOnShare() {
    const uniqueId = crypto.randomUUID();
    var optionOnShare = await SimpleOptionOnShare.connect(option_seller).deploy(
      uniqueId,
      "SimpleOptionOnShare",
      "SimpleOptionOnShare",
      option_buyer.address,
    ...
    const [tokenSymbol, premiumAmount] = await optionOnShare.premium();
    await premiumToken
      .connect(option_buyer)
      .approve(await optionOnShare.contractAddress(), premiumAmount);

    await premiumToken.allowance(
      option_buyer.address,
      optionOnShare.contractAddress()
    );
    // Buyer approve premium transfer, so we can do the deal.
    await optionOnShare.connect(option_buyer).acceptTerms();
    return optionOnShare;
  }
});
