...
describe("Secure Level Test Suite", function () {
  async function deployAccountAndToken() {
    ...
    const SecureLevel = await ethers.getContractFactory(
      "contracts/SecureLevel/SecureLevel.sol:SecureLevel",
      {
        libraries: {
    ...
    const secureLevel = await SecureLevel.connect(owner).deploy(
      symbol,
      description
    );
  ...
  describe("Secure Level Basics", async function () {
    it("Construction", async function () {
      var { secureLevel, owner, secure_source, other_source } =
        await loadFixture(deployAccountAndToken);
      const [symbol_, description_, live_, value_, lastUpdate_] =
        await secureLevel.getDetails();
  ...
  describe("Secure Level Security and Access control", async function () {
    it("Only owner can change verified source", async function () {
      ...
      await expect(
        secureLevel
          .connect(other_source)
          .setExpectedSigner(ethers.constants.AddressZero)
    ...
    it("Bad address passed as verified source", async function () {
      ...
      await expect(
        secureLevel
          .connect(owner)
          .setExpectedSigner(ethers.constants.AddressZero)
      ).to.be.revertedWith("SecureLevel: Bad expected signer address");
    });
    it("Only verified source can set level", async function () {
      var { secureLevel, owner, secure_source, other_source } =
        await loadFixture(deployAccountAndToken);
      var { value, nonce, secretMessage, secretMessageHash } =
        await loadFixture(signedMessage);
      ...
      ); // Don't forget to arrayify to send bytes
      await expect(
        secureLevel
          .connect(other_source)
          .setVerifiedValue(value, nonce, ethers.utils.arrayify(sig))
      ).to.be.revertedWith("SecureLevel: caller is not the verified signer");
    });
  });
  ...
  describe("Change of secure source", async function () {
    it("Valid change of verified source", async function () {
      var { secureLevel, owner, secure_source, other_source } =
        await loadFixture(deployAccountAndToken);
      await expect(
        secureLevel.connect(owner).setExpectedSigner(secure_source.address)
      )
  ...
  describe("Set level", async function () {
    it("Verified source must sign to set level", async function () {
      ...
      var sig = await owner.signMessage(
        ethers.utils.arrayify(secretMessageHash)
      ); // Signed as owner *not* secure source
      await expect(
        secureLevel
          .connect(secure_source)
          .setVerifiedValue(value, nonce, ethers.utils.arrayify(sig))
      ).to.be.revertedWith("SecureLevel: Required sender did not sign data");

      sig = await secure_source.signMessage(
        ethers.utils.arrayify(secretMessageHash)
    ...
    it("Set Value and check timestamp", async function () {
      ...
      var [symbol_, description_, live_, value_, lastUpdate_] =
        await secureLevel.getDetails();
      expect(live_).to.equal(false);
      ...
      const [actualValue, actualUpdated] = await secureLevel
        .connect(other_source)
        .getVerifiedValue();
      ...
      [symbol_, description_, live_, value_, lastUpdate_] =
        await secureLevel.getDetails();
      expect(symbol_).to.equal(symbol);
      expect(description_).to.equal(description);
  ...
  describe("Check update constraints", async function () {
    it("Greater than zero", async function () {
      ...
      var secretMessage = ethers.utils.solidityPack(
        ["uint256", "uint256"],
        [value, nonce]
      );
      var secretMessageHash = ethers.utils.keccak256(secretMessage);
      var sig = await secure_source.signMessage(
        ethers.utils.arrayify(secretMessageHash)
      );
      await expect(
        secureLevel
      ...
      value = 0;
      secretMessage = ethers.utils.solidityPack(
        ["uint256", "uint256"],
        [value, nonce]
      );
    ...
    it("Greater than equal zero", async function () {
      ...
      await secureLevel
        .connect(secure_source)
        .setVerifiedValue(value, nonce, ethers.utils.arrayify(sig));
      var [actualValue, actualUpdated] = await secureLevel
        .connect(other_source)
        .getVerifiedValue();
      expect(actualValue).to.equal(value);
      expect(actualUpdated).to.be.lessThan(Math.floor(Date.now()));
      ...
      await secureLevel
        .connect(secure_source)
        .setVerifiedValue(value, nonce, ethers.utils.arrayify(sig));
      [actualValue, actualUpdated] = await secureLevel
        .connect(other_source)
        .getVerifiedValue();
      expect(actualValue).to.equal(value);
      expect(actualUpdated).to.be.lessThan(Math.floor(Date.now()));
