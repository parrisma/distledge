...
async function exerciseERC721OptionNFT(
    ...
    console.log(`start exercise. 
            ...
            NFT Id:[${nftOptionId}], 
            Settlement FX:[${settlementFXLevelContract}], 
     ...
     */
    const currentOwner = await erc721OptionNFTContract.connect(managerAccount).ownerOf(Number(nftOptionId));
    if (currentOwner !== exerciseAddress) {
        throw new Error(`[${exerciseAddress}] is not current owner of [[${nftOptionId}]], so cannot exercise it, [${currentOwner}] is the owner`)
    }
    ...
    if (isNaN(settleFxRate)) {
        throw new Error(`Failed to get settlement FX rate for exercise from Level [${settlementFXLevelContract}]`);
    }
    const settleFXDecimals = await settlementFXLevelContract.connect(managerAccount).getDecimals();
    if (isNaN(settleFXDecimals)) {
        throw new Error(`Failed to get settlement Fx decimals for exercise from Level [${settlementFXLevelContract}]`);
    }
    const settleDecimals = await settlementTokenContract.connect(managerAccount).decimals();
    if (isNaN(settleDecimals)) {
        throw new Error(`Failed to get settlement decimals for exercise from Token [${settlementTokenContract}]`);
    }

    var settlementValue = ((settlementValue / (10 ** settleDecimals)) * (settleFxRate / (10 ** settleFXDecimals))) * (10 ** settleDecimals);
    ...
    if (newOwner !== issuerAddress) {
        throw new Error(`Failed to exercise [[${nftOptionId}]]`)
    }
