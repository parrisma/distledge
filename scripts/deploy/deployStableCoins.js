 ...
 * @param {Address} stableCoinIssuer The Signer account to use to issue the stable coins
 * @returns {Address} Of the StableCoin contracts that were deployed USD, EUR, CNY
 */
async function deployStableCoins(sharedConfig, hre, stableCoinIssuer) {

    console.log("\nIssue Stable Coins");
    const ERC20USDStableCoin = await hre.ethers.getContractFactory("ERC20USDStableCoin");
    erc20USDStableCoin = await ERC20USDStableCoin.connect(stableCoinIssuer).deploy();
    ...
    erc20EURStableCoin = await ERC20EURStableCoin.connect(stableCoinIssuer).deploy();
    ...
    erc20CNYStableCoin = await ERC20CNYStableCoin.connect(stableCoinIssuer).deploy();
