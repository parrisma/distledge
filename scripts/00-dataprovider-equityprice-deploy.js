...
async function main() {
  ...
  console.log(
    `Deploying Contract ${name} in network ${network.name} with chainid[${network.config.chainId}]...from ${dataFeedAddress}...`
  );
  const deployed = await contract.deploy(dataFeedAddress);
  console.log(`Contract deployed with address ${deployed.address}`);

  //Verify
  //Same as running in CLI => yarn hardhat verify --network goerli <contract_address> <constructor_args>
